...
def get_last(num, xt, yv):
    """
    identifies final n decay periods in list of decay_days.
    :param xt: list of decay days
    :param num: number of days
    :param yv: y-vals for decay days 
    """
    #start from end
...
def decay_params(bb, time_dim, spring, parallel = True, plot=True, return_vals=False):
    ...
    if any(((i < 0.7) & (i != 0)) for i in yv):
        ...
        ## VIS curve fit
        vis_model, cov = optimize.curve_fit(
            albedo_dev.albedo_vis_pwr,
            xdata=xt, 
            ydata=vis,
            maxfev=1000000,
            p0=[4300, 3600, 1.89],
        ...
        ## IR curve fit
        ir_model, cov = optimize.curve_fit(
            albedo_dev.albedo_ir_pwr,
            xdata=xt, 
            ydata=ir, 
            maxfev=1000000,
            p0=[100, 1000, 2],
        ...
        # so duplicate array is added (vis_model[3]) to return same amount of arrays
...
def split_season_wys(df, spring, last_events=False, n=3):
    """
    run decay_series and get_last for individual water years
    """
    xtl = []
    yvl = []
