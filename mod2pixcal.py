...
def decay_params(bb, time_dim, spring, parallel = True, plot=True, return_vals=False):
    ...
    if any(((i < 0.7) & (i != 0)) for i in yv):
        ...
        ## VIS curve fit
        vis_model, cov = optimize.curve_fit(albedo_dev.albedo_vis_2,
                                    xdata=xt, 
                                    ydata=vis,
                                    maxfev=1000000,
                                    p0=[40, 1000],
                                    bounds=((0, 0), (10000, 100000)))        
        ...
        if plot==True:
            # PLOT VISIBLE
            # not yet updated for albedo_vis_2
            ...
            y2 = albedo_dev.albedo_vis(x, 100, 1000, 500)
            y3 = vis
